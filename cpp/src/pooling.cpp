        ...
        for (int c = 0; c < channels; ++c) {
            for (int oh = 0; oh < out_height; ++oh) {
                for (int ow = 0; ow < out_width; ++ow) {
                    ...
                    output[b * channels * out_height * out_width + c * out_height * out_width + oh * out_width + ow] = max_val;
                    max_indices_[b * channels * out_height * out_width + c * out_height * out_width + oh * out_width + ow].push_back(max_idx);
                }
            }
        }
    }
...
void MaxPool2d::backward(Tensor& grad_output, Tensor& grad_input) {
    ...
    for (int b = 0; b < batch_size; ++b) {
        for (int c = 0; c < channels; ++c) {
            for (int oh = 0; oh < out_height; ++oh) {
                for (int ow = 0; ow < out_width; ++ow) {
                    int idx = b * channels * out_height * out_width + c * out_height * out_width + oh * out_width + ow;
                    int max_idx = max_indices_[idx][0];
                    int kh = max_idx / kernel_size_;
                    int kw = max_idx % kernel_size_;
                    int ih = oh * stride_ + kh;
                    int iw = ow * stride_ + kw;
                    if (ih < in_height && iw < in_width) {
                        grad_input[b * channels * in_height * in_width + c * in_height * in_width + ih * in_width + iw] =
                            grad_output[idx];
                    }
                }
            }
