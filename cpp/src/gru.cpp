...
GRU::GRU(int input_size, int hidden_size, int num_layers)
    ...
    for (int i = 0; i < num_layers; ++i) {
        input_to_hidden_.push_back(std::make_shared<Linear>(i == 0 ? input_size : hidden_size, hidden_size * 3));
        hidden_to_hidden_.push_back(std::make_shared<Linear>(hidden_size, hidden_size * 3));
...
void GRU::forward(Tensor& input, Tensor& output) {
    ...
    for (int t = 0; t < seq_len; ++t) {
        ...
        for (int l = 0; l < num_layers_; ++l) {
            ...
            Tensor r_gate({batch_size, hidden_size_});
            Tensor z_gate({batch_size, hidden_size_});
            Tensor n_gate({batch_size, hidden_size_});
            for (int b = 0; b < batch_size; ++b) {
                for (int j = 0; j < hidden_size_; ++j) {
                    r_gate[b * hidden_size_ + j] = gates[b * hidden_size_ * 3 + j];
                    z_gate[b * hidden_size_ + j] = gates[b * hidden_size_ * 3 + hidden_size_ + j];
                    n_gate[b * hidden_size_ + j] = gates[b * hidden_size_ * 3 + 2 * hidden_size_ + j];
                }
            }

            gate_activations_[l]->forward(r_gate, r_gate);
            gate_activations_[l]->forward(z_gate, z_gate);
            tanh_activations_[l]->forward(n_gate, n_gate);

            for (int b = 0; b < batch_size; ++b) {
                for (int j = 0; j < hidden_size_; ++j) {
                    h[b * hidden_size_ + j] = z_gate[b * hidden_size_ + j] * h[b * hidden_size_ + j] +
                                              (1.0f - z_gate[b * hidden_size_ + j]) * n_gate[b * hidden_size_ + j];
                }
            }
...
void GRU::backward(Tensor& grad_output, Tensor& grad_input) {
    ...
    for (int t = seq_len - 1; t >= 0; --t) {
        ...
        for (int l = num_layers_ - 1; l >= 0; --l) {
            ...
            Tensor grad_z({batch_size, hidden_size_});
            Tensor grad_n({batch_size, hidden_size_});
            for (int b = 0; b < batch_size; ++b) {
                for (int j = 0; j < hidden_size_; ++j) {
                    grad_z[b * hidden_size_ + j] = grad_h[b * hidden_size_ + j] * (h[b * hidden_size_ + j] - n_gate[b * hidden_size_ + j]);
                    grad_n[b * hidden_size_ + j] = grad_h[b * hidden_size_ + j] * (1.0f - z_gate[b * hidden_size_ + j]);
                }
            }
            ...
            for (int b = 0; b < batch_size; ++b) {
                for (int j = 0; j < hidden_size_; ++j) {
                    grad_gates[b * hidden_size_ * 3 + j] = 0.0f; // r_gate not used in simple GRU
                    grad_gates[b * hidden_size_ * 3 + hidden_size_ + j] = grad_z[b * hidden_size_ + j];
                    grad_gates[b * hidden_size_ * 3 + 2 * hidden_size_ + j] = grad_n[b * hidden_size_ + j];
                }
            }
...
void GRU::set_weights(const Tensor& weights) {
    weights_ = weights;
}
